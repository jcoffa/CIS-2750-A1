...
ICalErrorCode initializeDateTime(const char *line, DateTime *dt) {
    ...
    fprintf(stderr, "\tDEBUG: in initializeDateTime: line (length of %d) = \"%s\"\n", len, line);
    ...
    fprintf(stderr, "\tDEBUG: in initializeDateTime, data + strcspn(data, \"%s\") = \"%s\"\n", delimTime, data + strcspn(data, delimTime)+1);
...
ICalErrorCode initializeProperty(const char *line, Property **prop) {
    ...
    fprintf(stderr, "\tDEBUG: in initializeProperty: line = \"%s\"\n", line);
    ...
    if (firstDelim == length) {
        fprintf(stderr, "\tDEBUG: in initializeProperty: delim characters \"%s\" were not present in the line: \"%s\"\n", \
               delim, line);
    ...
    fprintf(stderr, "DEBUG: in initializeProperty: name=\"%s\", descr=\"%s\"\n", name, descr);
